...
def make_sbc(mapdl, x, y, z, tid="", cid="", tdof="111111", _type="rigid", pinb=None):
    ...
    if pinb:
        ...
        mapdl.nsel(type_="r", item="LOC", comp="X", vmin=0.0, vmax=pinb)
        mapdl.csys(0)
        mapdl.csdele(cs_id)
...
def get_mem(mapdl):
    ...
    for i in range(int(n_modes)):
        ...
        for j, d in enumerate(["X", "Y", "Z", "ROTX", "ROTY", "ROTZ"]):
            effmi[j + 1] = mapdl.get(
                entity="MODE", entnum=i + 1, item1="EFFM", item2="DIREC", it2num=d
            )
...
def get_sene_comps(mapdl, comp_names):
    ...
    for c in comp_names:
        ...
        df = pd.DataFrame(
            {"freq": df1["freq"].values, c: df1["sene"].values / sene_total}
        )
        dfs.append(df)
    df_sene = pd.concat(
        [d.set_index("freq") for d in dfs], axis=1, ignore_index=False, sort=True
    )
...
def plot_nodal_disp(grid, result, idx_set=0, dof="uZ", scale=2.0):
    ...
    u = pd.DataFrame(disp, index=nnum).reindex(gridn, fill_value=0.0)
...
def make_pv_array(
    ...
    mapdl.mp("EY", mid, 10)
    mapdl.mp("EZ", mid, Ez)
    mapdl.mp("GXY", mid, 10.0)
    mapdl.mp("GXZ", mid, Gxz)
    mapdl.mp("GYZ", mid, Gyz)
    mapdl.mp("PRXY", mid, 0.0)
    mapdl.mp("PRXZ", mid, 0.0)
    mapdl.mp("PRYZ", mid, 0.0)
    mapdl.mp("DENS", mid, dens)
    ...
    a1 = mapdl.blc4(xcorner=0.0, ycorner=-panel_w / 2, width=panel_h, height=panel_w)
    ...
    # base remote points
    mapdl.lsel(type_="s", item="LOC", comp="X", vmin=0.0)
    mapdl.nsll(type_="s", nkey=1)
    n_base_1, r_base = make_sbc(mapdl, 0.0, -panel_w / 4.0, 0.0, pinb=dx_panel)
    mapdl.lsel(type_="s", item="LOC", comp="X", vmin=0.0)
    mapdl.nsll(type_="s", nkey=1)
    n_base_2, r_base = make_sbc(mapdl, 0.0, panel_w / 4.0, 0.0, pinb=dx_panel)
    ...
    if n_panels > 1:
        for i in range(n_panels - 1):
            ...
            mapdl.nsll(type_="s", nkey=1)
            nr1, rr1 = make_sbc(
                mapdl, x_ref + dx_panel / 2.0, -panel_w / 4.0, 0.0, pinb=dx_panel
            )
            mapdl.lsel(type_="s", item="LOC", comp="X", vmin=x_ref)
            mapdl.nsll(type_="s", nkey=1)
            nr2, rr2 = make_sbc(
    ...
    df_mem = (
        get_mem(mapdl)
        .loc[:, ["mode", "TX", "TY", "TZ", "RX", "RY", "RZ"]]
        .set_index("mode")
    )
    df_sene_mem = pd.concat(
        [df_mem, df_sene], axis=1, ignore_index=False, sort=True
...
def data_bars(df, column, bar_max=1.0, bar_min=0.0, clr="#0074D9"):
    n_bins = 100
    bounds = [i * (1.0 / n_bins) for i in range(n_bins + 1)]
    ranges = [((bar_max - bar_min) * i) + bar_min for i in bounds]
    styles = []
        ...
        styles.append(
            {
                ...
                "background": (
                    ...
                    white 100%)
                """.format(
                        max_bound_percentage=max_bound_percentage, clr=clr
                    )
                ),
                "paddingBottom": 2,
                "paddingTop": 2,
                    ...
                    [
                        dbc.Form(
                            [
                                dbc.Label(
                                    "Number of Panels",
                                    html_for=f"{APP_ID}_n_panels_input",
                                ),
                                dbc.Input(
                                    id=f"{APP_ID}_n_panels_input",
                                    type="number",
                                    min=0,
                                    max=10,
                                    step=1,
                    ...
                    [
                        dbc.Form(
                            [
                                dbc.Label(
                                    "Panel Width (in)",
                                    html_for=f"{APP_ID}_w_panel_input",
                                ),
                                dbc.Input(
                                    id=f"{APP_ID}_w_panel_input",
                                    type="number",
                                    min=1.0,
                                    value=20,
                                    step="any",
                    ...
                    [
                        dbc.Form(
                            [
                                dbc.Label(
                                    "Panel Height (in)",
                                    html_for=f"{APP_ID}_h_panel_input",
                                ),
                                dbc.Input(
                                    id=f"{APP_ID}_h_panel_input",
                                    type="number",
                                    min=1.0,
                                    value=20,
                                    step="any",
                ...
                dbc.Col(
                    [
                        dbc.Form(
                            [
                                dbc.Label(
                                    "Panel Core Thickness (in)",
                                    html_for=f"{APP_ID}_tc_input",
                                ),
                                dbc.Input(
                                    id=f"{APP_ID}_tc_input",
                                    type="number",
                                    min=0.125,
                                    value=0.25,
                                    step="any",
        ...
        dbc.ButtonGroup(
            [
                dbc.Button(
                    ...
                    id=f"{APP_ID}_options_collapse_button",
                    color="secondary",
                ),
                                ...
                                dbc.Form(
                                    [
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    "Non Structural Mass (lbs/in^2)",
                                                    html_for=f"{APP_ID}_nsm_input",
                                                ),
                                                dbc.Input(
                                                    id=f"{APP_ID}_nsm_input",
                                                    type="number",
                                                    min=0.0,
                                                    value=0.002,
                                                    step="any",
                                        ...
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    ...
                                                    html_for=f"{APP_ID}_tfs_input",
                                                ),
                                                dbc.Input(
                                                    id=f"{APP_ID}_tfs_input",
                                                    type="number",
                                                    min=0.01,
                                                    value=0.030,
                                                    step="any",
                                ...
                                dbc.Form(
                                    [
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    "Hinge Effective Rotational Stiffness (in-lbs/rad)",
                                                    html_for=f"{APP_ID}_k_hinge_input",
                                                ),
                                                dbc.Input(
                                                    id=f"{APP_ID}_k_hinge_input",
                                                    type="number",
                                                    min=1000,
                                                    value=10e3,
                                                    step=1000,
                                    ...
                                    [
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    "Panel Separation (in)",
                                                    html_for=f"{APP_ID}_dx_panel_input",
                                                ),
                                                dbc.Input(
                                                    id=f"{APP_ID}_dx_panel_input",
                                                    type="number",
                                                    min=0.0,
                                                    value=2.0,
                                                    step="any",
                                        ...
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    ...
                                                    html_for=f"{APP_ID}_scale_input",
                                                ),
                                                dbc.Input(
                                                    id=f"{APP_ID}_scale_input",
                                                    type="number",
                                                    value=5.0,
                                                ),
                                            ],
                                    ...
                                    [
                                        html.Div(
                                            [
                                                dbc.Label(
                                                    "Show Edges",
                                                    html_for=f"{APP_ID}_show_edges_switch",
                                                ),
                                                dbc.Switch(
...
def dash_vtk_update_grid(
    ...
    res, ugrid, df_sene_mem = make_pv_array(
        panel_w,
        panel_h,
        n_panels=n_panels,
        nsm_pv=nsm_pv,
